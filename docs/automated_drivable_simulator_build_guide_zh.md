...
当前尚未完成统一的一键 CLI。连续长路线的数据审计已经完成；下一步只
下载选定第 2/3 块的 424.2 MB 数据，分别训练 100-step SplatAD 烟测，
并在共同的 20 m 区域用匹配世界位姿检查接缝。接缝通过后才训练更长
版本和实现 checkpoint 预加载/切换，同时把现有实验脚本逐步包装为本
说明书中的 `audit`、`train`、`validate`、`package` 和 `run` 阶段。
