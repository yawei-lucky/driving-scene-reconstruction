...
- 一条 610.07 m 的 TbV 同方向重复路线，已经划分为 7 个
  100 m、重叠 20 m 的真实相邻重建块。
- 第 2/3 块的 424.2 MB 数据、独立 100/500-step checkpoint 和相同
  世界位姿对照；道路结构对齐，但 500-step 仍不够清晰。

当前尚未完成统一的一键 CLI。下一步只把第 2/3 块训练到 2,000 step，
在共同的 20 m 区域生成连续切换/混合视频和 `-1/0/+1 m` 前视对照。
质量接缝通过后才训练其余五块和实现 checkpoint 预加载/切换，同时把
现有实验脚本逐步包装为本说明书中的 `audit`、`train`、`validate`、
`package` 和 `run` 阶段。
