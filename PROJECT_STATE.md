...
### Stage H3 Level 9O — MTGS simulated-driver closed loop

Completed on 2026-07-25 without training, browser control, or a physical
operator:

- connected a deterministic pure-pursuit/speed controller to `HumanControl`,
  `SimpleVehicleModel`, the actual-route support adapter, camera-pose sampler,
  and released MTGS renderer;
- accelerated from rest to 11.994 m/s, completed actual +2.541/-2.671 m
  vehicle-driven lateral excursions and recovery, and braked to rest at
  progress 83.8 m;
- produced 183 fixed-time 960x540 frames over 9.15 encoded seconds with zero
  boundary hits and 2.329 m minimum support margin;
- limited the maximum per-tick normalized steer change to 0.0636 and kept the
  maximum heading error at 16.675 degrees inside the reviewed 30-degree gate;
- explicitly measured and corrected the camera-right versus route-left sign
  alignment instead of assuming matching lateral coordinates;
- rendered every frame finite at 6.72/12.16 ms p50/p95 with 1.455 GiB peak
  reserved CUDA memory;
- found the maximum adjacent-frame image difference in a clustered recovery
  around the negative lateral extremum rather than an isolated route-knot
  jump;
- manually retained continuous road, lane, curb, and open-corridor evidence,
  while close foliage and edges remain soft or stretched.

This passes the simulated-driver vehicle/control/render loop for the existing
84 m block. It does not establish a long route, physical-human driving,
...
### Stage H3 Level 9Q — TbV adjacent-tile reconstruction seam pilot

Completed on 2026-07-25 in the accepted H3 SplatAD environment:

- downloaded the exact selected payload once: 2,805 unique objects and
  424,193,048 bytes, with per-object size validation and a saved manifest;
- extended the downloader with explicit repeatable windows and deduplicated
  overlapping objects;
- extended the TbV parser with per-traversal absolute end times, allowing the
  shared storage directory to produce independent tile-2 and tile-3 datasets;
- trained separate step-99 checkpoints for both tiles using both traversals,
  0.9 train split, 0.25 downsampling, and at most 250,000 seed points;
- rendered five exact shared reference-log front-camera timestamps through
  both checkpoints. All images were finite and metre scale, but coloured
  particles made the 100-step visual seam inconclusive;
- trained a bounded step-499 repeat for the same pair without expanding to the
  other tiles or 2,000 steps;
- improved tile-2/tile-3 observed-pose PSNR p50 from 15.115/15.442 dB to
  19.873/20.902 dB;
- reduced matched-pose model-to-model RGB MAE p50 from 26.566 to
  18.037 / 255, about 32%;
- manually recovered corresponding road, building, utility-pole, and horizon
  structure from both 500-step checkpoints without a coordinate/topology jump;
- retained visible blur, granularity, appearance differences, and
  undecomposed vehicles, so an instantaneous checkpoint switch is not yet
  accepted.

This passes the independent adjacent-tile integration and bounded promotion
gate. It does not pass the final visual seam, checkpoint switching,
counterfactual lateral views, or driving. Exact evidence is in
`experiments/stage_h3_tbv_adjacent_tile_seam.md`.

### Stage H3 Level 9R — TbV continuous adjacent-tile seam gate

Completed on 2026-07-25 in the accepted H3 SplatAD environment:

- trained only tiles 2 and 3 to step 1,999, then restored model, optimizer,
  scheduler, and global-step state for a bounded continuation to step 7,999;
- preserved exact-resume audits for both tiles. RNG and dataloader state are
  absent from the source checkpoint, so the resume is not bitwise equivalent
  to uninterrupted training;
- improved matched observed-pose PSNR p50 from 19.873/20.902 dB at 500 steps
  to 23.527/22.774 dB at 2,000 and 24.059/24.845 dB at 8,000;
- reduced matched-pose tile-to-tile RGB MAE p50 from 18.037 / 255 at 500 steps
  to 9.835 at 2,000 and 9.420 at 8,000;
- rendered the same 18.7522/18.7524 m interpolated world-pose path through both
  8k models at 12 m/s and 20 fps; all 32 frames were finite and both videos
  decoded completely;
- reduced the hard transition-frame RGB delta from 10.834 to 4.635 / 255 with
  a 5.626 m smoothstep blend;
- rendered `-1/0/+1 m` front views at the overlap start, middle, and end with
  no mostly-black result;
- manually retained aligned road, lane, building, utility-pole, wire, and
  horizon structure without a coordinate/topology jump or obvious blended
  double building edge;
- retained independently baked vehicles, dark floating blobs, and sharpness
  differences. A central vehicle-shaped smear remains a possible false
  obstacle, so pixel blending is not accepted as dynamic-scene truth;
- preserved the first partial 2k run, which exposed a nanosecond timestamp
  endpoint-rounding bug, then bounded the fix to 128 ns with regression tests.

This passes the static-background geometry seam for the selected pair. It does
not pass credible traffic, live checkpoint residency, the approximately 180 m
