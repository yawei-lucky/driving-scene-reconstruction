...
HEADLESS_TRIAL_ROOT="${H3_TBV_HEADLESS_TRIAL_ROOT:-${H3_ROOT}/artifacts/tbv_headless_humanized_20260725}"
...
case "$MODE" in
  ...
  headless-trial)
    ...
    "$PYTHON" "$REPO_ROOT/examples/stage_h3_tbv_headless_humanized_trial.py" \
      --config "$STATIC_CONFIG" \
      --output-dir "$HEADLESS_TRIAL_ROOT" \
      --output-scale "${H3_TBV_DRIVING_SCALE:-0.75}" \
      --max-speed-mps "${H3_TBV_MAX_SPEED_MPS:-4.0}" \
      --expected-checkpoint-step 7999
    ;;
  paths)
    ...
    echo "headless trial: $HEADLESS_TRIAL_ROOT"
    ;;
  *)
    echo "Usage: $0 {download|data-gate|smoke|render-smoke|pilot|render-pilot|static-8k|render-static-8k|world-pose-probe|world-pose-probe-8k|driving-adapter|headless-trial|paths}" >&2
    exit 2
    ;;
esac
